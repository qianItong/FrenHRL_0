...
def get_ego_past_array_from_scenario(scenario, num_past_poses, past_time_horizon, iteration=0):
    
    current_ego_state = scenario.get_ego_state_at_iteration(iteration)
    past_ego_states = scenario.get_ego_past_trajectory(
        iteration=iteration, num_samples=num_past_poses, time_horizon=past_time_horizon
    )
...
def get_ego_future_array_from_scenario(scenario, current_ego_state, num_future_poses, future_time_horizon,iteration=0):

    future_trajectory_absolute_states = scenario.get_ego_future_trajectory(
        iteration=iteration, num_samples=num_future_poses, time_horizon=future_time_horizon
    )
...
def calculate_additional_ego_states(ego_agent_past, time_stamp):
    ...
    yaw_rate = angle_diff / dt if dt > 0 else 0.0
