...
def get_single_reference_line(
    ...
    # if _last_relative_trajectory is None:
    #     trajectory = _last_relative_trajectory
    #     current_route_lane, chose_index, min_distance = get_cloeset_line_to_line(trajectory[:3],routes)
    #     if min_distance > 10:
    #         current_route_lane, chose_index, min_distance = get_cloeset_line_to_line(trajectory[:3].tolist(),all_lanes)
    ...
    while(find_next_route_lane):
        ...
        for i in range(len(routes)):
            ...
            if distance < 2: #and abs(vector_angle(vector, [1, 0]))<=100 and point2curve_nearest_point(current_route[-1][0], current_route[-1][1], reference_line)[0] > 3:
                find_next_route_lane = True
